...
If an Excel workbook already exists, newly generated values are merged
into the existing table. Previously exported values remain unless
replaced by newly computed non-null values.
...
def export_excel(
    ...
    If an existing workbook is present, newly generated values are
    merged into the existing table so that running individual
    processing pipelines updates only the corresponding columns.
    """
